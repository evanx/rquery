                     ...
                     switch (_context26.prev = _context26.next) {
                        case 0:
                           ...
                           if (Values.isDefined(req.query.archive)) {
                              result = ['mkdir -p ~/.redishub/archive', 'mv -n ~/.redishub/live ~/.redishub/archive/`date +\'%Y-%M-%dT%H:%M:%S@%s\'`'];
                           } else if (Values.isDefined(req.query.force)) {
                              result = ['rm -rf ~/.redishub/live'];
                           } else {
                              result = ['mkdir -p ~/.redishub'];
                           }
                           result = result.concat(['(', '  if mkdir ~/.redishub/live && cd ~/.redishub/live', '  then', '    echo \'' + account + '\' > account', '    if openssl req -x509 -nodes -days 365 -newkey rsa:2048 \\', '      -subj \'/CN=' + CN + '/OU=' + OU + '\' \\', '      -keyout privkey.pem -out cert.pem', '    then', '      cat privkey.pem cert.pem > privcert.pem', '      openssl x509 -text -in privcert.pem | grep \'CN=\'', '      curl -s -E privcert.pem ' + _this6.config.hostUrl + '/register-account-telegram/' + account + ' ||', '        echo \'Registered account ' + account + ' ERROR $?\'', '      echo; pwd; ls -l', '      curl -s -L https://raw.githubusercontent.com/evanx/redishub/master/docs/install.rhcurl.txt', '      if ! openssl pkcs12 -export -out privcert.p12 -inkey privkey.pem -in cert.pem', '      then', '        echo \'ERROR $?: openssl pkcs12 -export -out privcert.p12 -inkey privkey.pem -in cert.pem\'', '      fi', '    fi', '  fi', ')']);
                           result.push('');
                           result = result.concat(['Cut and paste the above directly into your shell.', 'To force archiving an existing ~/.redishub/live, add \'?archive\' to the URL', '   to first move ~/.redishub/live to ~/.redishub/archive/TIMESTAMP', 'Generate a password-protected P12 cert as follows to load into your browser:', '  cd ~/.redishub/live && [ -f privcert.p12 ] && openssl pkcs12 -export -out privcert.p12 -inkey privkey.pem -in cert.pem', 'Then use: ~/.redishub/live/privcert.pem (curl) and/or privcert.p12 (browser)', 'Create a keyspace called \'tmp-10days\' as follows:', '   curl -s -E ~/.redishub/live/privcert.pem ' + _this6.config.hostUrl + '/ak/' + account + '/tmp-10days/create-keyspace', 'See help for keyspace \'tmp-10days\' in your browser as follows:', '   ' + _this6.config.hostUrl + '/ak/' + account + '/tmp-10days/help', 'For CLI convenience, install rhcurl bash script, as per instructions:', '  curl -s -L https://raw.githubusercontent.com/evanx/redishub/master/docs/install.rhcurl.txt'].map(function (line) {
                              return '# ' + line;
                           }));
                           result.push('');

               ...
               while (1) {
                  switch (_context6.prev = _context6.next) {
                     ...
                     case 19:
                        ...
                        _context6.next = 24;
                        return this.sendTelegram(request.chatId, 'html', ['Thanks, ' + request.greetName + '.', 'Your RedisHub account name is <b>' + account + '</b>, as per your Telegram user.', 'Please try the bash commands generated by the following link to create a client cert:', this.config.hostUrl + '/generate-cert-script/' + account + '. ', ['<pre>', 'curl -s ' + this.config.hostUrl + '/generate-cert-script/' + account + ' | bash', '</pre>'].join(''), 'Add <code>?archive</code> to the URL to archive first if <code>~/.redishub/live</code>exists.', ['<pre>', '/generate-cert-script/' + account + '?archive', '</pre>'].join('')]);

         ...
         while (1) {
            switch (_context2.prev = _context2.next) {
               ...
               case 9:
                  ...
                  result = result.concat(['  if [ -d ' + dir + ' ] # directory already exists', '  then # must be archived first', '    echo "Directory ' + dir + ' already exists. Try add \'?archive\' query to the URL."', '  else # fetch, review and check SHA of static cert-script.sh for execution', '    mkdir ' + dir + ' && cd $_ # error exit if dir exists', '    curl -s https://raw.githubusercontent.com/webserva/webserva/master/bin/cert-script.sh -O', '    echo \'Please review and press Ctrl-C to abort within 8 seconds:\'', '    cat cert-script.sh # review the above fetched script, we intend to execute', '    echo \'Double checking script integrity hashes:\'', '    sha1sum cert-script.sh # double check its SHA against another source below', '    curl -s https://open.webserva.com/assets/cert-script.sh.sha1sum', '    echo \'184063fdc03f1b2ee3781d24dd8ace859bd86f89\' # hardcoded SHA of stable version', '    echo \'Press Ctrl-C in the next 8 seconds to abort, and if any of the above hashes differ\'', '    sleep 8 # give time to abort if SHAs not consistent, or script review incomplete', '    source <(cat cert-script.sh) # execute fetched script, hence the above review and SHA', '  fi', ')']);
                  result.push('');

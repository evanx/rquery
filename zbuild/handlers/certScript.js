         ...
         while (1) {
            switch (_context2.prev = _context2.next) {
               ...
               case 8:
                  ...
                  result = result.concat(['  if [ -d ' + dir + ' ]', '  then', '    echo "Directory ' + dir + ' already exists. Try add \'?archive\' query to the URL."', '  else', '    mkdir -p ' + config.clientCertHomeDir, '    cd $_', '    curl -s https://raw.githubusercontent.com/evanx/redishub/master/bin/cert-script.sh -O', '    cat cert-script.sh', '    sha1sum cert-script.sh', '    curl -s https://redishub.com/assets/cert-script.sh.sha1sum', '    echo \'Press Ctrl-C in the next 8 seconds if the above hashes do not match\'', '    sleep 8', '    mkdir ' + dir + ' && cd $_ # error exit if dir exists', '    source <(cat cert-script.sh)', '  fi', ')']);
                  result.push('');

         ...
         while (1) {
            switch (_context.prev = _context.next) {
               ...
               case 8:
                  ...
                  helpResult = ['', '  Find your privcerts: ' + dir + '/privcert.pem (curl) and/or privcert.p12 (browser)', '  For example, create a keyspace called \'tmp10days\' as follows:', '    ' + curlAccount + '/tmp10days/create-keyspace', '  Then try Redis commands on this keyspace for example:', '    ' + curlAccount + '/tmp10days/help', '    ' + curlAccount + '/tmp10days/set/mykey/myvalue', '    ' + curlAccount + '/tmp10days/get/mykey', '', '  In your browser, load \'privcert.p12\' and try:', '    ' + serviceUrl + '/ak/' + account + '/tmp10days/help', '', '  For CLI convenience, install wscurl bash script, as per instructions:', '    curl -s -L ' + docUrl + '/install.wscurl.txt', ''];
         ...
         while (1) {
            switch (_context2.prev = _context2.next) {
               ...
               case 9:
                  ...
                  result = result.concat(['  if [ -d ' + dir + ' ]', '  then', '    echo "Directory ' + dir + ' already exists. Try add \'?archive\' query to the URL."', '  else', '    mkdir ' + dir + ' && cd $_ # error exit if dir exists', '    curl -s https://raw.githubusercontent.com/webserva/webserva/master/bin/cert-script.sh -O', '    echo \'1c04b96bde8f4f1f1b4c05c9c368204bd8b46e54\'', '    cat cert-script.sh', '    sha1sum cert-script.sh', '    curl -s https://webserva.com/assets/cert-script.sh.sha1sum', '    echo \'Press Ctrl-C in the next 8 seconds if any of the above hashes differ\'', '    sleep 8', '    source <(cat cert-script.sh)', '  fi', ')']);
                  result.push('');

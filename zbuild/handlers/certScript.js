         ...
         while (1) {
            switch (_context.prev = _context.next) {
               ...
               case 8:
                  ...
                  result = ['', 'Curl this script and pipe into bash as follows:', '', 'curl -s \'' + serviceUrl + '/' + commandKey + '/' + account + '?dir=~/.redishub/live&noarchive\' | bash', ''].map(function (line) {
                     return '# ' + line;
                  });

                  result.push('');
                  result.push('(');
                  result = result.concat(['  account=\'' + account + '\'', '  CN=\'' + CN + '\' # unique client ID (service, account, role, id)', '  OU=\'' + OU + '\' # role for this cert', '  O=\'' + O + '\' # account name', '', '  dir=' + dir + ' # must not exist, or be archived', '  # Note that the following files are created in this dir:', '  # account privkey.pem cert.pem privcert.pem privcert.p12 x509.txt', '  commandKey=\'' + commandKey + '\'', '  serviceUrl=\'' + serviceUrl + '\'', '  archive=' + archive, '', '  certWebhook="${serviceUrl}/create-account-telegram/${account}"', '']);

         ...
         while (1) {
            switch (_context2.prev = _context2.next) {
               ...
               case 9:
                  ...
                  result = result.concat(['  if [ -d ' + dir + ' ]', '  then', '    echo "Directory ' + dir + ' already exists. Try add \'?archive\' query to the URL."', '  else', '    mkdir ' + dir + ' && cd $_ # error exit if dir exists', '    curl -s https://raw.githubusercontent.com/webserva/webserva/master/bin/cert-script.sh -O', '    cat cert-script.sh # review the script we intend to execute', '    sha1sum cert-script.sh # double check its SHA against another source below', '    curl -s https://open.webserva.com/assets/cert-script.sh.sha1sum', '    echo \'1c04b96bde8f4f1f1b4c05c9c368204bd8b46e54\' # hardcoded SHA of stable version', '    echo \'Press Ctrl-C in the next 8 seconds if any of the above hashes differ\'', '    sleep 8 # give time to abort if SHAs not consistent', '    source <(cat cert-script.sh) # execute fetched script, hence the above review and SHA', '  fi', ')']);
                  result.push('');

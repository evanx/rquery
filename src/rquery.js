...
export default class {
   ...
   async registerAccount(req, res) {
      try {
         ...
         const toptSecret = this.generateTokenKey();
         ...
         const [hsetnx, saddAccount, saddCert] = await redisClient.multiExecAsync(multi => {
            ...
            multi.sadd(this.adminKey('account', account, 'topt'), toptSecret);
   ...
   generateTokenCode(toptSecret, time) {
      time = time || new Date().getTime();
      logger.info(Object.keys(speakeasy));
   }

   buildQrReply(options) {
      let {label, account, user, host, toptSecret, issuer} = options;
      if (!toptSecret) {
         toptSecret = this.generateTokenKey();
      }
      logger.debug('code', this.generateTokenCode(toptSecret));
      if (!issuer) {
         issuer = label || host;
      }
      ...
      const uri = `${account}?secret=${toptSecret}&issuer=${issuer}`;
      ...
      return {toptSecret, uri, otpauth, googleChartUrl};
   }
